...
function SelectWrapped(input: Props) {
  const onChange = (value: string) => {
    input.onChange(value)
  }
  return (
    <div className={cn('flex gap-2.5 h-8', input.className)}>
      <Select onValueChange={onChange} defaultValue={input.initialValue}>
      ...
      <div className='text-xs flex flex-column justify-center'>
        <div className='m-auto'>{input.label}</div>
      </div>
    </div>
  )
}
...
>(({ className, children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ref={ref}
    className={cn(
      'flex h-8 w-full items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-xs ring-offset-background data-[placeholder]:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 cursor-pointer disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1',
      className,
    )}
    {...props}
  >
